<template>
	<div class="position">
		...
		<div class="more-list">
			<h5>省/市列表</h5>
			<ul>
				<li><a href="#" v-for="position in positions" :key="position" @click="getCity(position)">{{position}}</a></li>
			</ul>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		data () {
			return {
				positions:['北京','天津','河北','山西','内蒙古','辽宁','吉林','黑龙江','上海','江苏','浙江','安徽','福建','江西','山东',
'河南','湖北','湖南','广东','广西','海南','重庆','四川','贵州','云南','西藏','甘肃','青海','宁夏','新疆','台湾','香港','澳门']
			}
		},
		computed:{
        	...mapState({
